...
zup::Zup::Zup(std::string device,uint32_t address)
{
  ...
  //this->readCommand(s.str());
  PM_INFO(_logZup," Value read "<<_value);
}
zup::Zup::~Zup()
{
  if (fd1>0)
...
void zup::Zup::readCommand(std::string cmd)
{
  if (fd1<0 || portstatus!=1)
    {
      PM_ERROR(_logZup," Device not open ");
      return;
    }
  ...
  if(rv == -1)
    {
      ...
      PM_ERROR(_logZup," select failed");
    }
  else if(rv != 0)
    {
  ...
  while (1)
	...
	{
	  PM_ERROR(_logZup," select empty "<<ntry);
...
web::json::value zup::Zup::Status()
{
  ...
  this->INFO();
  web::json::value r;
  r["vset"]=json::value::number(_vSet);
  r["vout"]=json::value::number(_vRead);
  r["iset"]=json::value::number(_iSet);
  r["iout"]=json::value::number(_iRead);
  r["status"]=json::value::number(_status);
  return r;
}
void zup::Zup::INFO()
{
