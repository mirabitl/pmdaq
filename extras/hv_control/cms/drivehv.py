...
def calVset(Vreq,P,T,unit="K"):
    P0=990
    T0=293.15
    corr=0.2+0.8*(P/P0*T0/(T))
    
    if (unit!="K"):
        corr=0.2+0.8*(P/P0*T0/(T+273.15))
...
class pico_monitor:
    ...
    def on_message(self,client, userdata, message):
        ...
        if (message.topic == "pico_dome/pulsalys_outlet/bme"):
            self.load_settings(self.csv_file)
            self.P=r_m["content"]["P"]/100.
            self.T=r_m["content"]["T"]
            for k in self.channels.keys():
                vreq,pcent=calVset(self.channels[k]["vset"],self.P,self.T,unit="C")
