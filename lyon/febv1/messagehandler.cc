...
febv1::messageHandler::messageHandler() : _npacket(0) {_sockMap.clear();}

void febv1::messageHandler::processMessage(NL::Socket* socket) 
{
  // build id

  const std::lock_guard<std::mutex> lock(_sem);
  uint64_t id=( (uint64_t) utils::convertIP(socket->hostTo())<<32)|socket->portTo();
  PM_DEBUG(_logFebv1,"Message received from "<<socket->hostTo()<<":"<<socket->portTo()<<" =>"<<std::hex<<id<<std::dec);
  ...
  while (size_remain>0)
  {
    ...
    _npacket++;
    // if (_npacket%1000 ==1)
    PM_DEBUG(_logFebv1,"Packet "<<_npacket<<" receive "<<ier<<" bytes from"<<std::hex<<id<<std::dec);
  ...
  if (icmd==_handlers.end())
    {
      PM_ERROR(_logFebv1,"Message received from "<<socket->hostTo()<<":"<<socket->portTo()<<" =>"<<std::hex<<id<<std::dec<<std::flush);
...
void febv1::messageHandler::removeSocket(NL::Socket* socket)
{
  uint64_t id=((uint64_t) utils::convertIP(socket->hostTo())<<32)|socket->portTo();
  std::map<uint64_t, ptrBuf>::iterator itsock=_sockMap.find(id);
...
void febv1::messageHandler::addHandler(uint64_t id,MPIFunctor f)
{
