...
gricv1::slcHandler::slcHandler(std::string ip) : socketHandler(ip,gricv1::Interface::PORT::SLC)
{
  _msg=new gricv1::Message();
}

void gricv1::slcHandler::sendSlowControl(uint8_t* slc,uint16_t lenbytes)
{
  uint16_t hrlen=lenbytes;
  uint16_t cpl32bit=4-hrlen%4;
  uint16_t len=hrlen+cpl32bit+gricv1::Message::Fmt::PAYLOAD+1;
  ...
  sp[0]=htons(len);
  _msg->ptr()[gricv1::Message::Fmt::CMD]=gricv1::Message::command::SLC;
  memcpy(&(_msg->ptr()[gricv1::Message::Fmt::PAYLOAD]),slc,hrlen);
  _msg->ptr()[len-1]=')';
  uint32_t tr=this->sendMessage(_msg);
}


bool gricv1::slcHandler::processPacket()
{
