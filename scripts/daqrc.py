...
class daqControl:
    def __init__(self,config_file):
        ...
        self.daqfsm.add_transition('initialise','CREATED','INITIALISED',after='daq_initialising',conditions='isConfigured')
        ...
        self.daqfsm.add_transition('destroy','CONFIGURED','CREATED',after='daq_destroying',conditions='isConfigured')
    ...
    def getStoredState(self):
         pl=json.loads(sac.executeCMD(self.session.pns_host,8888,"/PNS/SESSION/LIST",{}))
         for x in pl["REGISTERED"]:
             if (x.split(':')[0]==self.session.session):
                 return x.split(":")[1]
    ...
    def storeState(self):
        ...
        p["session"]=self.session.session
        p["state"]=self.state
        sac.executeCMD(self.session.pns_host,8888,"/PNS/SESSION/UPDATE",p)
        self.stored_state=self.getStoredState()
        self.fsmStatus()
        return
