...
web::json::value  caen::HVCaenInterface::ChannelInfo(uint32_t slot,uint32_t channel)
{
  if (isConnected())
    {
      json::value rep;
      rep["name"]=json::value::string(U(GetStringValue("Name",slot,channel)));
      rep["vset"]=json::value::number(GetFloatValue("V0Set",slot,channel));
      rep["iset"]=json::value::number(GetFloatValue("I0Set",slot,channel));
      rep["vmon"]=json::value::number(GetFloatValue("VMon",slot,channel));
      rep["imon"]=json::value::number(GetFloatValue("IMon",slot,channel));
      rep["rup"]=json::value::number(GetFloatValue("RUp",slot,channel));
      rep["status"]=json::value::number(GetIntValue("Status",slot,channel));
      return rep;
    }
  else
    {
      ...
      rep["imon"]=json::value::number(-1.0);
      rep["rup"]=json::value::number(-1.0);
      rep["status"]=json::value::number(9999);
      return rep;
