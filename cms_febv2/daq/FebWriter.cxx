...
FebWriter::FebWriter(std::string dire, std::string location) : _directory(dire), _location(location), _run(0), _idx(0), _fdOut(-1), _totalSize(0), _event(0), _useShm(false), _shmDirectory("") {}
void FebWriter::newRun(uint32_t run, bool resetEventNumber)
{
  _run = run;
...
uint32_t FebWriter::totalSize() { return _totalSize; }
uint32_t FebWriter::eventNumber() { return _event; }
uint32_t FebWriter::runNumber() { return _run; }
void FebWriter::setIds(uint32_t did,uint32_t sid)
{_detectorId=did;_sourceId=sid;}

void FebWriter::writeRunHeader(std::vector<uint32_t> header)
{
...
uint32_t FebWriter::appendEventData(std::vector<uint32_t> val)
{
...
std::string FebWriter::file_name() { return _file_name; }
bool FebWriter::useShm() { return _useShm; }
void FebWriter::setShmDirectory(std::string shd)
{
  _shmDirectory = shd;
  _useShm = true;
}