...
void Febv1Manager::c_cal6bdac(http_request m)
{
  ...
  for (auto it2 = querym.begin(); it2 != querym.end(); it2++)
  {
    ...
    if (it2->first.compare("shift") == 0)
      shift = std::stoi(it2->second);
  }
...
void Febv1Manager::c_scurve(http_request m)
{
  ...
  uint32_t first = 420, last = 530, step = 2, mode = 255,mask=0xFFFFFFFF;
  auto querym = uri::split_query(uri::decode(m.relative_uri().query()));
  for (auto it2 = querym.begin(); it2 != querym.end(); it2++)
  {
    ...
    if (it2->first.compare("step") == 0)
      step = std::stoi(it2->second);
    if (it2->first.compare("channel") == 0)
      mode = std::stoi(it2->second);
    if (it2->first.compare("mask") == 0)
      mask = std::stoi(it2->second);
  ...
  _sc_step = step;
  _sc_mask = mask;
  PMF_INFO(_logFebv1, "SCURVE called " << mode << " " << _sc_thmin << " " << _sc_thmax << " " << step <<" "<<std::hex<<mask<<std::dec);
  if (_sc_running)
  {
...
void Febv1Manager::fsm_initialise(http_request m)
{
  ...
  if (jFEBV1.as_object().find("json") != jFEBV1.as_object().end())
  {
    ...
    else if (jFEBV1json.as_object().find("url") != jFEBV1json.as_object().end())
    {
...
void Febv1Manager::ScurveStep(std::string mdcc, std::string builder, int thmin, int thmax, int step)
{
  ...
  web::json::value p;
  p["nclock"] = json::value::number(ncon);
  utils::sendCommand(mdcc, "SPILLON", p);
  p["nclock"] = json::value::number(ncoff);
  utils::sendCommand(mdcc, "SPILLOFF", p);
  printf("Clock On %d Off %d \n", ncon, ncoff);
  p["value"] = json::value::number(4);
  utils::sendCommand(mdcc, "SETSPILLREGISTER", p);
  utils::sendCommand(mdcc, "CALIBON", json::value::null());
  p["nclock"] = json::value::number(ntrg);
  utils::sendCommand(mdcc, "SETCALIBCOUNT", p);
...
#undef USEFEBS
#ifdef USEFEBS
    for (auto x : _mpi->boards())
      if (x.second->data()->event() > firstEvent)
        firstEvent = x.second->data()->event();
#else
    ...
    while (lastEvent < (firstEvent + ntrg - 10))
    {
      ::usleep(100000);
      for (auto x : _mpi->boards())
        if (x.second->data()->event() > lastEvent)
          lastEvent = x.second->data()->event();
      nloop++;
