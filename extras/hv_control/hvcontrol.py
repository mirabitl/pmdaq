...
class hv_monitor:
    ...
    def on_message(self,client, userdata, message):
        ...
        if (message.topic == self.config["dp_temperature"]):
            self.T=r_m["content"]["T"]    
        if (message.topic == self.config["dp_pressure"] and self.T!=0):
            ...
            for k in self.channels.keys():
                ...
                topic_cmd=self.config["dp_wiener"]
...
if __name__ == "__main__":
    s=hv_monitor("/etc/hvcontrol.json")
    s.Connect()
    s.ListTopics()
    s.loop()
    while 1:
