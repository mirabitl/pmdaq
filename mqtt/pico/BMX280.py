...
class BMX280():
    ...
    def _gauge(self):
        ...
        if utime.ticks_diff(now, self._last_read_ts) > self._new_read_ms:
            ...
            if self._chip_id != BMX280_BMP_CHIP_ID:
                r=4
                self._write(BME280_REGISTER_CONTROL_HUM, r)
            ...
            if self._chip_id == BMX280_BMP_CHIP_ID:
                ...
                print(self._p_raw,self._t_raw,self._h_raw)
            self._t_fine = 0
            self._t = 0
            self._h = 0
            self._p = 0
    ...
    def humidity(self):
        if self._chip_id == BMX280_BME_CHIP_ID:
            ...
            var1= self._t_fine - 76800.
            var2= ((self._H4) * 64.0 + (self._H5 / 16384.0) * var1)
            var3=self._h_raw-var2
            var4=self._H2/65536.0
            var5=(1.0 + (self._H3 / 67108864.0) * var1)
            var6= 1.0 + (self._H6 / 67108864.0) * var1 * var5;
            var6 = var3 * var4 * (var5 * var6);

            return  (var6 * (1.0 - self._H1 * var6 / 524288.0));
        else:
