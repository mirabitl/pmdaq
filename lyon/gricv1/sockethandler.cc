...
gricv1::socketHandler::socketHandler(std::string ip,uint32_t port) : _idx(0),_transaction(0)
{
...
void gricv1::socketHandler::clear()
{
  for (int i=0;i<255;i++)
    {
...
uint32_t gricv1::socketHandler::sendMessage(gricv1::Message* m)
{
  ...
  try
  {
    ...
    m->ptr()[gricv1::Message::Fmt::TRANS]=tr;
    // Clear the ack tag for reply
    _answ[tr][gricv1::Message::Fmt::CMD]=0;
...
int16_t gricv1::socketHandler::checkBuffer(uint8_t* b,uint32_t maxidx)
{
   uint32_t elen=0;
  if (b[0]!='(')  return -2;
 // Check TAG coherency
 if (b[0]=='(')
   {
     ...
     if (elen>maxidx)
       {
         PM_INFO(_logGricv1,"CheckBuf header:Not enough data ELEN "<<elen<<" MAXID "<<maxidx);
         return -1;
       }
     if (b[elen-1]==')')
       {
       ...
       {
         PM_INFO(_logGricv1,"CheckBuf header :Missing  end tag , found "<<b[elen-1] );
